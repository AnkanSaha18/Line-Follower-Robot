...
void lineFollow()
{
    ...
    {
        wheel(0, 0); // stop condition
    }
    else if (leftSensors && middleSensors && rightSensors)
    {
        wheel(0, 0);
        delay(10000);
    }
    else if ((middleSensors && leftSensors) || (IRValue[0] + IRValue[1] + IRValue[2] + IRValue[3] + IRValue[4] >= 4))
    {
        ...
        // int IRValue[SensorCount];
        // readIRSensor(IRValue);
        // if (IRValue[0] || IRValue[1] || IRValue[2] || IRValue[3] || IRValue[4] || IRValue[5] || IRValue[6] || IRValue[7] || IRValue[8] || IRValue[9])
        // {
        //     wheel(-60, 60);
        //     delay(200);     //delay for line out
        // }
        wheel(-55, 55); // optimum: 70
        wait();
        // delay(500);
    }
    ...
    // else if(leftSensors && rightSensors)
    // {
    //     wheel(0, 0);
    //     delay(5000);
    // }

    else if (leftSensors && rightSensors == 0)
    {
        wheel(-52, 52);
    }
